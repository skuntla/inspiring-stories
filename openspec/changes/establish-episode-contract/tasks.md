...
- [x] 6.3 Author the production series `series/willow-meadow/` (Tales from Willow Meadow: narrator, pip, ben (formerly bramble), wren; example-meadow style; `defaults.made_for_kids: false`) with its example episode, commit the rendered brief as `series/willow-meadow/chatgpt-instructions.md`, and add a test that every repo series validates, and that each committed brief matches a fresh render; verify `pytest` passes

## 7. Verification

- [ ] 7.1 End-to-end ChatGPT round trip: install the output of `story brief willow-meadow` as ChatGPT Project instructions, then (a) brainstorm and confirm you get prose and a storyboard with no YAML; (b) say "Lock this story" while the story still needs a character who is not in the cast and confirm ChatGPT asks only that question; also confirm it uses the series default `made_for_kids: false` without asking; (c) answer it and confirm one YAML block comes back; paste it into a `story new willow-meadow ...` episode and run `story validate`; (d) request a revision and relock, and confirm a complete replacement manifest that validates. Record the results in the change folder as `notes.md`
